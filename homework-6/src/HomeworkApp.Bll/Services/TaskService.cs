...
public class TaskService : ITaskService
{
        ...
        CancellationToken token)
    {
        ...
        if (!string.IsNullOrEmpty(cachedTask))
        {
            var taskMessages = JsonSerializer.Deserialize<TaskMessage[]>(cachedTask);
        ...
        var result = comments.Select(tc => new TaskMessage
        {
            TaskId = tc.TaskId,
            Comment = tc.Message,
            IsDeleted = tc.DeletedAt != null,
            At = tc.At
        }).ToArray();

        var commentJson = JsonSerializer.Serialize(result);
        await _distributedCache.SetStringAsync(
            cacheKey, 
            commentJson,
            new DistributedCacheEntryOptions
